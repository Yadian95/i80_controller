...
#if CONFIG_EXAMPLE_LCD_TOUCH_ENABLED
static void
example_lvgl_touch_cb(lv_indev_drv_t *drv, lv_indev_data_t *data)
{
...
void app_main(void)
{
    ...
    lv_fs_file_t f;
    lv_fs_open(&f, "A:foo.txt", LV_FS_MODE_RD);

    uint8_t buf[64];
    lv_fs_read(&f, buf, sizeof(buf), NULL);
    ...
    // lv_img_set_src(ui_notes, "A:MMC.bin");
    ...
    while (1)
    {
        ...
        // ESP_ERROR_CHECK(mpu9250_register_read(0x17, data, 2));
